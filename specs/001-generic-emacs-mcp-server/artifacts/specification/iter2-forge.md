...
- **POST**: Accepts JSON-RPC messages (requests, notifications, or responses). The request body is a single JSON-RPC message or a batch array.
  - If the input contains only notifications or responses, the server SHALL return HTTP 202 Accepted with no body.
  - If the input contains requests, the server SHALL return either `Content-Type: application/json` (single JSON response) or `Content-Type: text/event-stream` (SSE stream). The server chooses SSE when the response may be deferred or when server-initiated messages are needed.
- **GET**: Opens an SSE stream for server-to-client messages (notifications, requests). The server SHALL return `Content-Type: text/event-stream` or HTTP 405 if SSE is not supported.
...
- `initialize` — Return server capabilities, protocol version (`2025-03-26`), and server info. The response SHALL include `Mcp-Session-Id` header.
- `notifications/initialized` — Handle as a JSON-RPC notification (no response). Transition session state from "initializing" to "ready."
- `tools/list` — Return all registered tools with JSON Schema `inputSchema` objects. Supports pagination via `cursor` parameter.
...
The macro SHALL:
- Define a function `emacs-mcp-tool-my-tool-name--handler` as the handler.
...
**FR-2.4**: The package SHALL provide `emacs-mcp-register-tool` as a programmatic function alternative, accepting keyword arguments: `:name` (string), `:description` (string), `:params` (list of param plists), `:handler` (function).
...
**FR-2.7**: Tool dispatch SHALL validate required arguments before calling the handler:
- If the tool name is unknown: return a JSON-RPC **protocol error** with code `-32602` ("Unknown tool: <name>").
...
**FR-4.1**: On `initialize`, the server SHALL generate a UUID v4 session ID and return it in the `Mcp-Session-Id` response header. All subsequent requests from that client MUST include this header. Requests without a valid `Mcp-Session-Id` (other than `initialize`) SHALL receive HTTP 400 Bad Request.
...
- `project-dir` (string): Determined from the `rootUri` or `workspaceFolders` field in `initialize` params. If not provided, defaults to `default-directory` at server start time.
...
- `deferred` (hash-table): Maps JSON-RPC request ID (integer) to a pending deferred response context.
...
**FR-5.1**: Some tool handlers need user interaction before they can return (e.g., a confirmation dialog, a diff review). These tools use the **deferred response** pattern:

1. The tool handler returns the symbol `deferred` instead of a string or content list.
2. The server stores the JSON-RPC request ID in the session's `deferred` hash-table, keyed by request ID.
...
4. When the deferred action completes (e.g., user accepts/rejects), the completing code calls `emacs-mcp-complete-deferred` with the session, request ID, and result.
5. The server writes the JSON-RPC response as an SSE `data:` event on the open stream, then closes the stream.

**FR-5.2**: `emacs-mcp-complete-deferred` is a public function:
```elisp
(emacs-mcp-complete-deferred SESSION-ID REQUEST-ID RESULT &optional IS-ERROR)
```
- `SESSION-ID`: String, the session's UUID.
- `REQUEST-ID`: Integer, the JSON-RPC request ID.
...
**FR-5.3**: Deferred responses SHALL time out after `emacs-mcp-deferred-timeout` seconds (defcustom, default 300 / 5 minutes). On timeout, the server SHALL send a tool execution error response ("Deferred operation timed out") on the SSE stream and close it.

**FR-5.4**: If the SSE stream disconnects before the deferred response completes, the deferred entry SHALL remain in the session for the duration of the timeout. If the client reconnects (e.g., via GET), the server MAY deliver the response on the new stream. If the timeout expires without delivery, the deferred entry is discarded.
...
**FR-6.2**: `emacs-mcp-stop` SHALL gracefully shut down the server: send HTTP 404 to any active sessions, close all network connections and SSE streams, cancel all timers, remove all lockfiles, and clean up all session state.
...
**FR-6.5**: Neither `(require 'emacs-mcp)` nor loading the file SHALL install any hooks, modify any global state, or start any processes. All side effects are deferred to `emacs-mcp-start` or enabling `emacs-mcp-mode`.
...
- The server SHALL validate the `Origin` header on all incoming requests to prevent DNS rebinding attacks. Requests with an `Origin` header that is not `http://127.0.0.1:*` or `http://localhost:*` SHALL be rejected with HTTP 403.
...
**NFR-7: No Global State Pollution** — Per the constitution. The package SHALL not modify any global keymaps, hooks, or variables outside the `emacs-mcp-` namespace. All side effects are gated behind `emacs-mcp-start` / `emacs-mcp-mode`.
...
**AC-10**: `Origin` header validation rejects requests from non-localhost origins.
...
9. **JSON-RPC batching** — The server MAY accept batched requests but is not required to optimize for them in v1.
