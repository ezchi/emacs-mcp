...
- **POST**: Accepts a single JSON-RPC message (request, notification, or response). The request body MUST be a single JSON-RPC object, NOT a batch array. If a JSON array is received, the server SHALL return a JSON-RPC error with code `-32600` (Invalid Request) and message "Batch requests are not supported." The `initialize` request MUST NOT be batched (per MCP spec).
  - If the input is a notification or response, the server SHALL return HTTP 202 Accepted with no body.
  - If the input is a request, the server SHALL return either `Content-Type: application/json` (single JSON response) or `Content-Type: text/event-stream` (SSE stream). The server chooses SSE when the response may be deferred or when server-initiated messages are needed.
...
- `initialize` — Return server capabilities, protocol version (`2025-03-26`), and server info. The response SHALL include `Mcp-Session-Id` header. Server capabilities SHALL declare `tools: { listChanged: false }` (and `resources`, `prompts` as empty objects if supported).
...
- `emacs-mcp--current-request-id` — the JSON-RPC request ID (integer) of the current `tools/call`.
...
9. **JSON-RPC batching** — The server does NOT support batch requests in v1. Batch arrays are rejected with a JSON-RPC error (see FR-2.7). Batch support may be added in a future version.
