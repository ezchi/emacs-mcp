...
emacs-mcp.el (entry point)
  ...
  │   └── emacs-mcp-tools-builtin.el (depends on: tools for deftool, session for project-dir context)
...
- UUID v4 generation: read 16 bytes from `/dev/urandom` via `(with-temp-buffer (set-buffer-multibyte nil) (insert-file-contents-literally "/dev/urandom" nil 0 16) (buffer-string))`. Set version nibble (byte 6 → `#x40 | (logand byte #x0f)`) and variant bits (byte 8 → `#x80 | (logand byte #x3f)`) per RFC 4122. Format as `xxxxxxxx-xxxx-4xxx-Nxxx-xxxxxxxxxxxx`. This uses OS-level cryptographic randomness, not Emacs `random`. If `/dev/urandom` is not available (non-Unix systems), signal `user-error`: `"emacs-mcp: cannot generate secure session IDs (no /dev/urandom)"`. No non-cryptographic fallback — session IDs must be unpredictable per the spec
...
**P3.1** `emacs-mcp-protocol.el` — MCP method handlers (implement FIRST, no transport dependency)
...
- `emacs-mcp--handle-tools-call` — Validate, dispatch, bind dynamic vars, handle deferred. Reject null request IDs per MCP spec.
...
**P3.2** `emacs-mcp-transport.el` — Streamable HTTP transport (implement SECOND, depends on protocol)
- `emacs-mcp--transport-validate-session` — Shared session validation for all non-initialize POST/GET/DELETE. Extract `Mcp-Session-Id` header: missing → HTTP 400, syntactically invalid → HTTP 400, unknown/expired/terminated → HTTP 404.
- `emacs-mcp--transport-handle-post` — Route POST per MCP Streamable HTTP spec:
  ...
  3. For non-initialize requests, call `emacs-mcp--transport-validate-session`
  4. If body contains ONLY notifications/responses → HTTP 202 Accepted, no body
  5. If body contains requests → dispatch to protocol, return JSON or SSE
  6. Accept header is NOT validated (server is lenient per FR-1.2)
- `emacs-mcp--transport-handle-get` — Open SSE stream. Call `validate-session`. Register stream in session's `sse-streams`.
- `emacs-mcp--transport-handle-delete` — Terminate session. Call `validate-session`. HTTP 200 + cleanup.
...
- Batch request handling: detect batch array, process each message sequentially, collect responses. If ANY request triggers deferred, switch entire batch to SSE mode.
...
- Null request ID rejection: handled in protocol layer (P3.1). JSON-RPC requests with `"id": null` → protocol error `-32600` ("Null request IDs not allowed").
...
- Path authorization via `emacs-mcp--check-path-authorization` helper (calls `file-in-directory-p`). This function takes a path and reads the session's `project-dir` from `emacs-mcp--current-session-id` (dynamic var bound during tool execution, see FR-5.1). Session-context contract: built-in tools access the session's project directory via `(emacs-mcp-session-project-dir (emacs-mcp--session-get emacs-mcp--current-session-id))` to scope path checks, buffer filtering, and project info.
...
- `emacs-mcp-start` — Interactive command: **idempotent** — if server is already running (checked via `emacs-mcp--server-process`), return existing port without starting a second server (FR-6.1). Otherwise: validate port, clean stale lockfiles, start HTTP server, create lockfiles, run hooks
...
- Hooks: `emacs-mcp-server-started-hook` (run at end of `emacs-mcp-start`), `emacs-mcp-server-stopped-hook` (run at end of `emacs-mcp-stop`), `emacs-mcp-client-connected-hook` (run in `emacs-mcp--handle-initialize` after session creation), `emacs-mcp-client-disconnected-hook` (run in session cleanup: DELETE handler, timeout expiry, or server stop)
...
- ERT tests: `emacs-mcp-test-integration.el` — end-to-end tests covering all acceptance criteria:
  ...
  - AC-9 is covered by the per-module unit test files, NOT the integration test: `emacs-mcp-test-tools.el` (registration, unregistration, schema gen), `emacs-mcp-test-jsonrpc.el` (request parsing, response generation, null ID rejection), `emacs-mcp-test-protocol.el` (tool dispatch, argument validation), `emacs-mcp-test-session.el` (session create, lookup, timeout, cleanup), `emacs-mcp-test-tools-builtin.el` (path authorization, built-in tool behavior), `emacs-mcp-test-tools-builtin.el` (each built-in tool), `emacs-mcp-test-http.el` (HTTP parsing, request accumulation)
...
Phase 3: MCP Protocol + Transport
  P3.1 protocol (depends on: tools, session, jsonrpc) — implement FIRST
  P3.2 transport (depends on: http, jsonrpc, session, protocol) — implement SECOND
