...
## C-6: Lockfile cleanup on stale processes [NO SPEC CHANGE]

**Clarification**: FR-1.7 creates lockfiles with the Emacs PID. If Emacs crashes without running `kill-emacs-hook`, stale lockfiles remain. On startup, `emacs-mcp-start` SHOULD check for existing lockfiles at the target port, verify the PID is still alive (via `(process-attributes PID)`), and remove stale lockfiles before creating new ones. This is implementation guidance, not a spec change — the spec already requires lockfile creation/removal in FR-1.7 and FR-6.2.
