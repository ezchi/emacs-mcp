...
- Add `(require 'emacs-mcp-session)` to `emacs-mcp-tools.el`. This is
  necessary because test files load `emacs-mcp-tools` independently and
  call `emacs-mcp--dispatch-tool`. A `declare-function` would suppress
  byte-compiler warnings but leave `emacs-mcp--session-get` undefined
  at runtime in test contexts. No circular dependency exists:
  `emacs-mcp-session` requires `emacs-mcp`; `emacs-mcp-tools` already
  requires `emacs-mcp`.
...
**Where:** `emacs-mcp-tools.el` line 18 (add require), lines 22-26
(add variable), lines 233-234 (add session lookup + binding in
`emacs-mcp--dispatch-tool`).

**Risk:** Low — additive change; existing tools do not use the new
variable and remain correct. The new `require` adds a hard dependency
on `emacs-mcp-session` but this is already an implicit runtime
dependency.
