...
- Bind it to the session's `project-dir` at dispatch time (alongside
  `emacs-mcp--current-session-id` at line 233)
...
**Where:** `emacs-mcp-tools.el` lines 22-26 (add variable), lines
233-234 (add binding in `emacs-mcp--dispatch-tool`).

**Risk:** Low — additive change; existing tools do not use the new
variable and remain correct.
...
- AC-2: setProjectDir changes session's project-dir; then call
  `project-info` tool and verify it returns the new directory (proves
  subsequent tool calls use the new project-dir for scoping)
