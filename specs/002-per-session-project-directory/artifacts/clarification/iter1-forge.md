...
## CLR-5: Deferred Operations During Project Directory Change [NO SPEC CHANGE]
...
## CLR-6: Error Messages Shall Not Leak Allowlist [NO SPEC CHANGE]
...
`listChanged`). The Emacs Lisp side reads this via
`(alist-get 'projectDir params)` since `json-parse-string` converts
JSON keys to symbols by default.
